...
/**
 * A minimal bundle shape for extracting NHS numbers from FHIR bundles.
 */
export interface FhirBundleLike {
  entry?: Array<{
    resource?: {
      resourceType?: string;
      identifier?: Array<{value?: string}>;
      subject?: {identifier?: Array<{value?: string}>};
    };
  }>;
}

/**
 * Find the patient NHS number in a FHIR bundle by inspecting Patient and MedicationRequest entries.
 *
 * @param bundle - A FHIR bundle
 * @returns The NHS number if found, otherwise undefined
 */
export function findNhsNumber(bundle: FhirBundleLike): string | undefined {
  for (const entry of bundle.entry ?? []) {
    const resource = entry.resource;
    if (!resource) continue;

    if (resource.resourceType === "MedicationRequest") {
      const value = resource.subject?.identifier?.[0]?.value;
